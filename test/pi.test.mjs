...
import { parsePiVersion, parseInstalledPackages, parseInstalledPackageEntries, parseListModels, resolveWorkshopPackageSource, WORKSHOP_PACKAGE_SOURCE, createPi } from "../src/pi.mjs";

test("workshop package source is the npm spec Pi installs", () => {
  assert.equal(WORKSHOP_PACKAGE_SOURCE, "npm:@cuny-ai-lab/pi-workshop");
});
...
test("parseInstalledPackages reads package sources from pi list output", () => {
  const output = [
    "User packages:",
    "  npm:@cuny-ai-lab/pi-workshop",
    "    /Users/x/.pi/agent/npm/node_modules/@cuny-ai-lab/pi-workshop",
...
test("createPi runs pi through the platform spawn plan with argument arrays", () => {
  ...
  assert.deepEqual(calls[1].args, ["install", "npm:@cuny-ai-lab/pi-workshop"]);
...
test("parseInstalledPackageEntries pairs each source with its installed path when printed", () => {
  ...
  assert.deepEqual(parseInstalledPackageEntries(output), [
    { source: "npm:@cuny-ai-lab/pi-workshop", installedPath: "/Users/x/.pi/agent/npm/node_modules/@cuny-ai-lab/pi-workshop" },
...
test("hasWorkshopPackage recognizes a local-path install by the installed package's name", () => {
  const dir = mkdtempSync(join(tmpdir(), "pi-workshop-pkg-"));
  writeFileSync(join(dir, "package.json"), JSON.stringify({ name: "@cuny-ai-lab/pi-workshop" }));
